 ...
 * - OR the `userId` parameter is in the `likedByUserIds` array
 */
const findRelevantComments = (comments, userId) => {
  return comments.filter((comment) => {
    const { commenterId, likedByUserIds } = comment;

    return commenterId === userId || likedByUserIds.includes(userId);
  });
};
